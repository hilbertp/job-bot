# Philipp Hilbert - *Diplomatic Product Leadership in Software*
...
10+ years of XP · based in Limassol, Cyprus and Berlin, Germany · open to remote and to relocation · available at short notice
...
A product decision is not finished when the story is refined. After discovery and requirements, a coherent development and test-automation setup has to exist: a branching and merging strategy that allows a hotfix on the same day, regression tests the team genuinely trusts at release, and GitOps processes that turn rollout and rollback into a non-event, today almost always in containerised environments. Building and owning that chain is the work of architects and DevOps engineers, not mine. What I bring is understanding it well enough to steer it into the right lanes, to ask the right questions early, and to notice when discovery, requirements, test automation, delivery and operations start drifting apart. The time from spotting a defect to shipping the fix is a product metric, not an engineering detail.
...
Owned a module of the central AI backbone for a European defence programme, transforming multimodal data sources into datasets for machine learning training and evaluation.
...
My approach is AI-first, and it changes the economics of product work measurably. With AI development tools and a human in the loop I deliver, part-time, the output a four-person team needed full-time a year ago: requirements, interface design, frontend and backend, automated regression and end-to-end testing, architecture, infrastructure and operations. I do not write the production code myself, I own it. For that I built and run a multi-agent framework in which coordinator, implementer and evaluator agents work a shared queue under guardrails I designed.

From this follows a thesis I work by. Deep specialisation is losing weight, because the current frontier models already cover and largely replace it. What gains weight is breadth and transfer knowledge across the entire software value chain: the ability to see discovery, requirements, architecture, test automation, delivery and operations as one system and to intervene at the right point. Working this close to development is also what taught me firsthand how much those structures carry, which is why I treat them as product work rather than as somebody else's housekeeping.
